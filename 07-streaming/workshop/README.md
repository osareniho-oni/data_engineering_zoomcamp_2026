...
1. Start all Docker services (Redpanda, PostgreSQL, Flink)
2. Create the PostgreSQL table
3. Submit the Flink streaming job
4. Run the producer to send 1000 taxi rides
5. Verify the data in PostgreSQL

**Expected output:** ~1000 records in the `processed_events` table

**To start fresh:**
```bash
docker compose down -v  # Remove all data
./run_pipeline.sh       # Run again
```
...
- A SQL client (optional) - [pgcli](https://www.pgcli.com/) (`uvx pgcli`), DBeaver, pgAdmin, or DataGrip

## Manual Setup (Step by Step)

If you prefer to run each step manually or want to understand the process:
...
```bash
docker compose exec postgres psql -U postgres -d postgres -c "SELECT COUNT(*) FROM processed_events;"
```
...
- View running jobs
- Monitor metrics (records sent/received)
...
NYC Taxi Data (Parquet)
        ↓
Python Producer (src/producers/producer.py)
        ↓
Kafka/Redpanda (Topic: rides)
        ↓
...
| Redpanda Admin | 8083 | Admin interface |
...
├── Dockerfile.flink             # Custom Flink image with PyFlink
...
│   └── job/
│       ├── pass_through_job.py # Main Flink streaming job
│       └── aggregation_job.py  # Aggregation example
└── README.md                    # This file
```
...
docker compose logs redpanda   # Check Redpanda logs
docker compose logs jobmanager # Check Flink logs
```

### Port conflicts
If port 8083 is in use, modify `docker-compose.yml` to use a different port.

### Job failing
Check Flink dashboard at http://localhost:8081 for error details.

### No data in PostgreSQL
1. Ensure Flink job is running (check dashboard)
2. Verify producer completed successfully
3. Check table exists: `\dt` in psql
...
The sections below walk through building everything from scratch - you can follow
